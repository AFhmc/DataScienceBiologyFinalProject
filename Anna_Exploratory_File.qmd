...
Make a more general version that can handle multiple variables at once:
...
Make plot of variable against flux for all the sites

```{r}
variable <- "TA_F"

site_colors <- c("CA-NS2" = "#f92104", "CA-NS6" = "#0014f7")
...
yvar <- "RECO_NT_VUT_REF"

WW_data |> 
  filter(Site %in% c("CA-NS2", "CA-NS6"))|> 
  filter_variables(0.8, c(yvar, variable)) |>
            ggplot(aes(x = !!sym(variable), y = !!sym(yvar), color = Site)) +
            geom_point( alpha = 0.5) + 
            geom_smooth(se = T) + scale_alpha(guide = "none") +
            scale_color_manual(values = site_colors) + 
            labs(x = variable, y = yvar)

```


To get evapotransporation I'm using the LE.to.ET function from the bigleaf package (which is designed to work with eddy covariance data)
```{r}
library(bigleaf)
...
HH_data <-HH_data |> mutate(ET = LE.to.ET(LE_F_MDS, TA_F), ET_QC = LE_F_MDS_QC)
```
